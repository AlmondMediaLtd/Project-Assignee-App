...
extension App {
    ...
    static var currentView : UIView = UIView()
    ...
    static func displayNotification(msg : String)
    {
        if var topController = UIApplication.sharedApplication().keyWindow?.rootViewController {
            ...
            if let wnd = topController.view{
                
                let v = UIView(frame: wnd.bounds)
                v.backgroundColor = UIColor(red: 1.0 , green: 1.0, blue: 1.0, alpha: 0.2)
                    v.alpha = 1.0;
                
                wnd.addSubview(v)
                UIView.animateWithDuration(0.7, animations: {v.alpha = 0.0 }, completion: {(finished:Bool) in
                        
                    UIView.animateWithDuration(0.4, animations: { v.alpha = 1.0 }, completion: {(finished:Bool) in
                        
                        UIView.animateWithDuration(0.3, animations: { v.alpha = 0.0 }, completion: {(finished:Bool) in
            ...
            //topController.view.makeToast(msg, duration: 1.0, position: .Top)
        }
